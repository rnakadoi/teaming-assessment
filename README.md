...
GitHub Actions（`.github/workflows/backup.yml`）が毎週月曜 5:00 JST に `pg_dump` を実行し、`public`（言える化）と `metamo`（事業変態力診断）の両スキーマを gzip + GPG（AES256 対称鍵）で暗号化して Artifact として保存する（保持 90 日）。手動実行は Actions タブ → DB Backup → Run workflow。

**リポジトリが public のため Artifact は閲覧者がダウンロードできる。ダンプの暗号化は必須**（リポジトリを private 化した場合のみ省略可）。

### 必要な GitHub Secrets（Settings → Secrets and variables → Actions）

| Secret | 内容 |
|---|---|
| `SUPABASE_DB_URL` | Supabase ダッシュボード → Connect → **Session pooler** の URI（`aws-0-ap-northeast-1.pooler.supabase.com:5432`、パスワード込み）。direct connection は IPv6 のみのため GitHub Actions から接続不可 |
| `BACKUP_PASSPHRASE` | ダンプ暗号化用パスフレーズ（強いランダム文字列をパスワードマネージャーに保管。紛失するとバックアップを復号できない） |

### 復元手順

```bash
# 1. Actions → 対象 run → Artifact をダウンロード・展開（backup.sql.gz.gpg）
# 2. 復号・解凍
gpg --decrypt --batch --pinentry-mode loopback \
    --passphrase "$BACKUP_PASSPHRASE" backup.sql.gz.gpg | gunzip > dump.sql
# 3. 復元（SUPABASE_DB_URL は Session pooler の URI）
